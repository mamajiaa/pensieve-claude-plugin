...
BEGIN {
    ...
    cats[6] = "loop";      cat_title["loop"] = "Loop"
}
{
    if (FNR == 1) {
        file_allowed = 0
...
END {
    ...
    print "- Root: `" root "`"
    if (include_loop == "1") print "- Included categories: maxims, decisions, knowledge, pipelines, loop"
    else print "- Included categories: maxims, decisions, knowledge, pipelines"
    print ""
    print "```mermaid"
    print "graph LR"
