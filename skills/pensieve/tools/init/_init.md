...
---
description: 初始化项目级 `.claude/pensieve/` 用户数据目录并种子化基础文件（幂等，不覆盖已有文件）
---
...
- 需要沉淀经验或改进流程（应转 `/selfimprove`）

### Required inputs

- `<SYSTEM_SKILL_ROOT>`（由 SessionStart 注入）
- 项目根路径（当前仓库）
...
- 明确说明 review pipeline 默认依赖项目内 `.claude/pensieve/knowledge/`
...
3. 核验最小结果：
   - `.claude/pensieve/{maxims,decisions,knowledge,pipelines,loop}` 已存在
   - `.claude/pensieve/pipelines/run-when-reviewing-code.md` 已存在
   - `.claude/pensieve/knowledge/taste-review/content.md` 已存在
4. 若扫描到历史目录（如 `skills/pensieve/` 或 `.claude/skills/pensieve/`），追加提醒：请运行 `/upgrade` 处理迁移与清理。
