...
---
description: 初始化项目级 `.claude/skills/pensieve/` 并补齐种子文件；随后执行一次基线探索（提交记录+实际代码），产出可沉淀候选，并基于该结果调用 review pipeline 做品味分析。幂等且不覆盖已有用户数据；涉及迁移/清理必须先走 `/upgrade`。
---
...
- 仓库无提交记录或 Git 不可用：跳过探索与品味分析，明确标记 `SKIPPED`
...
- "把探索候选直接写入 knowledge/decision" → 转 `/self-improve`
...
8. 收尾输出：
   ...
   - 可沉淀候选摘要
   - 品味分析摘要
   - 若需落库，明确下一步：运行 `/self-improve`

## 约束

- 初始化可附带只读探索与品味分析，但不直接落库沉淀内容。
- 不做迁移清理（迁移只走 `/upgrade`）。
- 不覆盖已有用户文件。
