...
Hard rule：不要硬编码规范。每次执行都必须先读取规范文件，再从规范推导检查项。
...
约束：
- frontmatter 语法错误（未闭合、格式损坏）至少判为 `MUST_FIX`。
- frontmatter 缺失、必填字段缺失或字段值非法，判为 `MUST_FIX`。
- pipeline 命名违规（`FM-301/FM-302`），判为 `MUST_FIX`。
- `decision` 探索减负缺失（`FM-401~FM-404`），至少判为 `SHOULD_FIX`。
- 未运行此快检不得输出最终结论。
...
- doctor 阶段禁止自动改项目用户数据文件；仅 `SKILL.md` 自动维护块可更新。
- `decision` 或 `pipeline` 的必填链接在图谱中表现为断链，至少判为 `MUST_FIX`。
